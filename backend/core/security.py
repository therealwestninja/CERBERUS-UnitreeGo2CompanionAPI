...
class AuditLog:
    ...
    def record(self, event: str, source: str, data: dict,
        ...
        # Chain HMAC — use hmac.new (stdlib) with explicit digestmod
        entry_bytes = str(sorted(
            (k, v) for k, v in entry.items()
            if k != 'hash'  # exclude hash field from its own computation
        )).encode()
        entry['hash'] = hmac.new(
            self._secret, entry_bytes, hashlib.sha256
        ).hexdigest()
