"""
backend/main.py  — CERBERUS v3.2
=================================
Restored from ver_7_1. FastAPI application: REST + WebSocket.
Full endpoint set matching the original architecture.

The server receives bridge + engine via the lifespan factory.
...
async def lifespan(app: FastAPI):
    ...
    await engine.start()
    logger.info("CERBERUS API ready  (simulation=%s)", os.getenv("GO2_SIMULATION","false"))
...
async def root():
    eng = _require_engine()
    return {
        "service": "CERBERUS", "version": "3.2.0",
...
async def get_anatomy():
    eng = _require_engine()
    if eng.anatomy is None: raise HTTPException(404, "Anatomy subsystem not loaded")
    return eng.anatomy.status()

@app.get("/behavior")
async def get_behavior():
    eng = _require_engine()
    if eng.behavior_engine is None: raise HTTPException(404, "Behavior engine not loaded")
    return eng.behavior_engine.status()

@app.get("/plugins")
async def list_plugins():
    if plugin_manager is None: raise HTTPException(503, "Plugin manager not ready")
    return plugin_manager.list_plugins()

@app.get("/safety/events")
async def safety_events(n: int = 50):
    if watchdog is None: raise HTTPException(503, "Watchdog not ready")
...
async def stand_up():
    _require_engine(); _require_no_estop()
    watchdog.ping_heartbeat()
    if not await bridge.stand_up(): raise HTTPException(500, "stand_up failed")
    return ok()
...
async def stand_down():
    _require_engine(); _require_no_estop()
    watchdog.ping_heartbeat()
    await bridge.stand_down(); return ok()

@app.post("/motion/stop")
async def stop_motion():
    _require_engine()
    watchdog.ping_heartbeat()
    await bridge.stop_move(); return ok()

@app.post("/motion/move")
async def move(cmd: MoveCmd):
    _require_engine(); _require_no_estop()
    ok_v, reason = watchdog.validate_velocity(cmd.vx, cmd.vy, cmd.vyaw)
    if not ok_v: raise HTTPException(422, f"Velocity validation failed: {reason}")
    watchdog.ping_heartbeat()
...
async def body_height(cmd: BodyHeightCmd):
    _require_engine(); _require_no_estop()
    watchdog.ping_heartbeat()
    await bridge.set_body_height(cmd.height); return ok({"height_offset": cmd.height})

@app.post("/motion/euler")
async def set_euler(cmd: EulerCmd):
    _require_engine(); _require_no_estop()
    watchdog.ping_heartbeat()
    await bridge.set_euler(cmd.roll, cmd.pitch, cmd.yaw); return ok()

@app.post("/motion/gait")
async def switch_gait(cmd: GaitCmd):
    _require_engine(); _require_no_estop()
    await bridge.switch_gait(cmd.gait_id); return ok({"gait_id": cmd.gait_id})

@app.post("/motion/foot_raise")
async def foot_raise(cmd: FootRaiseCmd):
    _require_engine(); _require_no_estop()
    await bridge.set_foot_raise_height(cmd.height); return ok()

@app.post("/motion/speed_level")
async def speed_level(cmd: SpeedCmd):
    _require_engine(); _require_no_estop()
    await bridge.set_speed_level(cmd.level); return ok({"level": cmd.level})

@app.post("/motion/continuous_gait")
async def continuous_gait(cmd: ContinuousGaitCmd):
    _require_engine()
    await bridge.set_continuous_gait(cmd.enabled); return ok({"enabled": cmd.enabled})

@app.post("/motion/sport_mode")
async def sport_mode(cmd: SportModeCmd):
    _require_engine(); _require_no_estop()
    watchdog.ping_heartbeat()
    if not await bridge.execute_sport_mode(cmd.mode):
...
async def unload_plugin(name: str):
    if plugin_manager is None: raise HTTPException(503, "Plugin manager not ready")
    if not await plugin_manager.unload_plugin(name):
        raise HTTPException(404, f"Plugin '{name}' not found")
    return ok()
...
async def _handle_ws_command(ws: WebSocket, data: dict) -> None:
    ...
    if cmd == "move":
        if watchdog and not watchdog.estop_active:
            watchdog.ping_heartbeat()
            await bridge.move(data.get("vx",0.0), data.get("vy",0.0), data.get("vyaw",0.0))
    elif cmd == "stop":
        await bridge.stop_move()
    elif cmd == "estop":
        if watchdog: await watchdog.trigger_estop("WebSocket client")
    elif cmd == "sport_mode":
        if watchdog and not watchdog.estop_active:
            try:
