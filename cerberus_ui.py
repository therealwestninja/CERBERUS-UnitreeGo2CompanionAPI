...
# ── DPG tag constants — every registered item has a unique tag ────────────────
#
# TAG_HEADER_STATUS  lives in the top header bar (tick counter display).
# TAG_TICK_COUNT     lives in the RUNTIME table in the right column.
# These are intentionally different strings to prevent DPG duplicate-item errors.
...
TAG_TICK_COUNT      = "txt_ticks"           # runtime stats table — right column
TAG_OVERRUNS        = "txt_overruns"
TAG_QUEUE_DEPTH     = "txt_queue"
TAG_HEADER_STATUS   = "txt_header_status"   # header bar ticker — unique, not txt_ticks
...
class CERBERUSApp:
    ...
    Battery bar theming
    -------------------
    DPG progress bars cannot have their bar colour changed via configure_item().
    Three per-item themes (green / amber / red) are pre-built in
    _build_battery_themes() and swapped onto TAG_BATTERY each frame when the
    percentage bracket changes.  This avoids per-frame DPG theme creation and
    is the documented DPG approach for dynamic widget colours.
    """
    ...
    def __init__(self, bridge: UIBridge | None = None) -> None:
        ...
        # Battery bar themes — populated in _build_battery_themes()
        self._bat_theme_green: int = 0
        self._bat_theme_amber: int = 0
        self._bat_theme_red:   int = 0
        self._bat_last_level:  str = ""   # "green" | "amber" | "red"
    ...
    def _apply_theme(self) -> None:
        # noqa: SIM117 — DPG requires nested with blocks: outer creates the
        # theme object needed by bind_theme(); inner selects the component scope.
    ...
    def _build_battery_themes(self) -> None:
        """Pre-build three progress-bar themes for green / amber / red battery levels."""
        def _make(bar_colour: tuple) -> int:
            with dpg.theme() as theme:
                with dpg.theme_component(dpg.mvProgressBar):
                    dpg.add_theme_color(dpg.mvThemeCol_PlotHistogram, bar_colour,
                                        category=dpg.mvThemeCat_Core)
            return theme

        self._bat_theme_green = _make(GREEN)
        self._bat_theme_amber = _make(AMBER)
        self._bat_theme_red   = _make(RED)
    ...
    def _build_header(self) -> None:
        with dpg.group(horizontal=True):
            ...
            # TAG_HEADER_STATUS is a unique tag — different from TAG_TICK_COUNT
            dpg.add_text("", tag=TAG_HEADER_STATUS, color=TXT_DIM)
        dpg.add_separator()
    ...
    def _build_right_column(self) -> None:
        with dpg.child_window(width=-1, height=-1, border=True):
            ...
            dpg.add_spacer(height=6)
            with dpg.group(horizontal=True):
                dpg.add_button(label=" ▶  PLAY  ",  tag=TAG_PLAY_BTN,
                               callback=self._cb_play,  width=90)
                dpg.add_button(label=" ⏸  PAUSE ", tag=TAG_PAUSE_BTN,
                               callback=self._cb_pause, width=90)
                dpg.add_button(label=" ⏹  STOP  ", tag=TAG_STOP_BTN,
                               callback=self._cb_stop,  width=90)
            ...
            with dpg.group(horizontal=True):
                # noqa: SIM117 — DPG requires nested with for theme -> component
                with dpg.theme() as estop_theme:
                    with dpg.theme_component(dpg.mvButton):
                        dpg.add_theme_color(dpg.mvThemeCol_Button,
                                            (200, 30, 30, 255), category=dpg.mvThemeCat_Core)
                        dpg.add_theme_color(dpg.mvThemeCol_ButtonHovered,
                                            (230, 50, 50, 255), category=dpg.mvThemeCat_Core)
                        dpg.add_theme_color(dpg.mvThemeCol_ButtonActive,
                                            (255, 80, 80, 255), category=dpg.mvThemeCat_Core)
                        dpg.add_theme_style(dpg.mvStyleVar_FrameRounding,
                                            4, category=dpg.mvThemeCat_Core)
                ...
                with dpg.theme() as clear_theme:
                    with dpg.theme_component(dpg.mvButton):
                        dpg.add_theme_color(dpg.mvThemeCol_Button,
                                            (30, 120, 60, 255), category=dpg.mvThemeCat_Core)
                        dpg.add_theme_color(dpg.mvThemeCol_ButtonHovered,
                                            (40, 150, 75, 255), category=dpg.mvThemeCat_Core)
                        dpg.add_theme_color(dpg.mvThemeCol_ButtonActive,
                                            (55, 170, 90, 255), category=dpg.mvThemeCat_Core)
            ...
            with dpg.table(header_row=False, borders_innerH=False,
                ...
                dpg.add_table_column()
                for label, tag in [
                    ("Ticks",     TAG_TICK_COUNT),    # "txt_ticks"
    ...
    def _update(self) -> None:
        ...
        # Dataclass content comparison — identity check alone would always differ
        if (self._last_state is not None
                and state.__dict__ == self._last_state.__dict__):
            return
    ...
    def _apply_state(self, s: UIState) -> None:
        ...
        # Uses TAG_HEADER_STATUS ("txt_header_status"), NOT TAG_TICK_COUNT
        dpg.set_value(TAG_HEADER_STATUS, f"tick {s.tick_count}")
        ...
        # ── Battery (value + colour) ──────────────────────────────────────────
        self._update_battery(s.battery_pct, s.battery_voltage)
        ...
        hr_color = (
            RED   if s.hr_alarm
            else AMBER if s.heart_rate_bpm > 150
            else TXT_MAIN
        )
        ...
        dpg.set_value(
            TAG_FS_PATH,
            Path(s.fs_path).name if s.fs_loaded and s.fs_path else "No file loaded",
        )

        if s.fs_duration_ms > 0:
            dpg.set_value(TAG_FS_PROGRESS, s.fs_position_ms / s.fs_duration_ms)
            dpg.set_value(
                TAG_FS_POSITION,
                f"{s.fs_position_ms/1000:.1f}s / {s.fs_duration_ms/1000:.1f}s"
                f"  ({s.fs_position_norm * 100:.0f}%)",
            )
        else:
            dpg.set_value(TAG_FS_PROGRESS, 0.0)
            dpg.set_value(TAG_FS_POSITION,  "—")

        dpg.configure_item(TAG_PLAY_BTN,
                           enabled=s.fs_loaded and not s.fs_playing and not s.estop)
        dpg.configure_item(TAG_PAUSE_BTN, enabled=s.fs_playing)
        dpg.configure_item(TAG_STOP_BTN,  enabled=s.fs_loaded)

        # ── Plugins table — rebuild only when content changes ─────────────────
        if s.plugin_states != self._last_plugins:
            ...
            for name, state_name in s.plugin_states.items():
                plug_color = (
                    CYAN    if state_name == "ACTIVE"
                    else RED if state_name == "ERROR"
                    else TXT_DIM
                )
                with dpg.table_row(parent=TAG_PLUGIN_TABLE):
                    dpg.add_text(name)
                    dpg.add_text(state_name, color=plug_color)

        # ── Runtime stats — TAG_TICK_COUNT lives only in this table ───────────
    ...
    def _update_battery(self, pct: int, voltage: float) -> None:
        """
        Update battery value, overlay text, and bar colour.

        Colour is applied by swapping the per-item theme bound to TAG_BATTERY.
        DPG progress bars do not expose a colour kwarg via configure_item();
        binding a pre-built theme is the correct approach.  We only rebind
        when the level bracket changes to avoid unnecessary DPG calls per frame.
        """
        norm = pct / 100.0
        dpg.set_value(TAG_BATTERY, norm)
        dpg.configure_item(TAG_BATTERY, overlay=f"{pct}%")
        dpg.set_value(TAG_BATTERY_LABEL, f"{voltage:.1f} V")

        if norm > 0.30:
            level, theme = "green", self._bat_theme_green
        elif norm > 0.15:
            level, theme = "amber", self._bat_theme_amber
        else:
            level, theme = "red",   self._bat_theme_red

        if level != self._bat_last_level:
            dpg.bind_item_theme(TAG_BATTERY, theme)
            self._bat_last_level = level

    # ── Callbacks ─────────────────────────────────────────────────────────────

    def _cb_estop(self)       -> None: self._bridge.send_command("estop")
    def _cb_clear_estop(self) -> None: self._bridge.send_command("clear_estop")
    def _cb_play(self)        -> None: self._bridge.send_command("play")
    def _cb_pause(self)       -> None: self._bridge.send_command("pause")
    def _cb_stop(self)        -> None: self._bridge.send_command("stop")
    ...
    def _set_status(tag: str, connected: bool) -> None:
        dpg.set_value(tag, "● ONLINE" if connected else "○ offline")
        dpg.configure_item(tag, color=CYAN if connected else TXT_DIM)
